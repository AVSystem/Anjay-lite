 ...
 * returned, this function must be called again with the same arguments.
 *
 ...
 *
 * @return @ref ANJ_NET_OK or @ref ANJ_NET_EINPROGRESS on success, a negative
...
/**
 * Closes the connection to the server. If @ref ANJ_NET_EINPROGRESS is returned,
 ...
 *         value in case of an error.
 */
int _anj_srv_conn_close(_anj_server_connection_ctx_t *ctx, bool cleanup);

/**
 * Sends a message to the server. If this function returns error, connection
 * must be closed. If @ref ANJ_NET_EINPROGRESS is returned, this function must
 * be called again with the same arguments.
 *
 * @param ctx     Server connection context.
 ...
 *         value in case of an error.
 */
int _anj_srv_conn_send(_anj_server_connection_ctx_t *ctx,
 ...
 *
 * @return @ref ANJ_NET_OK, @ref ANJ_NET_EAGAIN or @ref ANJ_NET_EINPROGRESS on
 *         success, a negative value in case of an error.
 */
int _anj_srv_conn_receive(_anj_server_connection_ctx_t *ctx,
                          uint8_t *buffer,
                          size_t *out_length,
                          size_t length);
 ...
 * is returned, this function must be called again. If different value is
 * returned, the exchange is finished. Before the first use of this function,
 * the exchange must be already started with
 * @ref _anj_exchange_new_client_request or
 * @ref _anj_exchange_new_server_request. This function is integration layer
 * between exchange API and network API.
 *
 ...
 *
 * @return 0, @ref ANJ_NET_EAGAIN @ref ANJ_NET_EINPROGRESS on success,
 ...
 * ANJ_NET_EAGAIN or @ref ANJ_NET_EINPROGRESS.
 *
 * @param anj         Anjay instance.
 ...
 * @return 0 on success, a negative value in case of an error.
 */
int _anj_srv_conn_prepare_client_request(anj_t *anj,
...
/**
 * Switches the server connection to queue mode with RX off.
 *
 * @param ctx  Server connection context.
 *
 * @return 0 on success, a negative value in case of an error.
 */
int _anj_srv_conn_queue_mode_rx_off(_anj_server_connection_ctx_t *ctx);

#endif // ANJ_SRC_CORE_SRV_CONN_H
