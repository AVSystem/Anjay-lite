...
#define _ANJ_SRV_CONN_MINIMAL_BLOCK_SIZE 16
#define _ANJ_SRV_CONN_GENERIC_ERROR -1

static int net_again_is_error(int result) {
    return ((result == ANJ_NET_EAGAIN) || result == ANJ_NET_EINPROGRESS)
                   ? _ANJ_SRV_CONN_GENERIC_ERROR
                   : result;
}
...
int _anj_srv_conn_handle_request(anj_t *anj) {
    ...
    while (1) {
        ...
        if (anj->exchange_cache.handling_retransmission) {
            ...
            result = _anj_srv_conn_send(&anj->connection_ctx, anj->out_buffer,
                                        anj->out_msg_len);
