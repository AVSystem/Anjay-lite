...
static void fu_reset(void *user_ptr) {
    ...
    fu->download_active = false;
    fu->received_chunks = 0;
...
static void coap_downloader_callback(void *arg,
    ...
    case ANJ_COAP_DOWNLOADER_STATUS_SUSPENDED:
        log(L_INFO, "Firmware download suspended");
        break;
    case ANJ_COAP_DOWNLOADER_STATUS_RESUMING:
        log(L_INFO, "Firmware download resuming");
        break;
    ...
    case ANJ_COAP_DOWNLOADER_STATUS_FAILED:
        log(L_ERROR,
            "Firmware download failed with error: %d",
...
int fw_update_object_install(anj_t *anj,
    ...
    memset(&firmware_update, 0, sizeof(firmware_update));
    ...
    if (anj_dm_fw_update_object_install(
                anj, &fu_entity, &fu_handlers, &firmware_update)) {
        return -1;
    }
