...
// Prepare @ref anj_net_config_t for @ref anj_coap_downloader_start()
// by populating net_cfg with certificate info from Security Object.
// Setting CERT still allows for no-sec downloads if uri contains "coap://".
//
// NOTE: this example assumes that DTLS connection can be established using
// exactly the same credentials as those used for the LwM2M Server.
// This works with Coiote DM but may NOT apply to all servers or deployment
// models. If your download server requires separate credentials, modify
// this logic to select the appropriate credentials.
