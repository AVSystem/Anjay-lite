...
INCLUDE_PUBLIC_PATH = PROJECT_ROOT / "include_public/anj"
CONFIG_IN_RELATIVE = Path("anj_config/anj/anj_config.h")
CONFIG_OUT = PROJECT_ROOT / "build/doc/doxygen/config/include_public/anj/anj_config.h"


def stage_files(doxygen_input_dir):
    """
    Copies include_public headers into doxygen_input_dir/include_public.
    """
    dest_include_public = doxygen_input_dir / 'include_public/anj'
    ...
    # 3. Copy the cmake-configured config and patch it
    if (PROJECT_ROOT / CONFIG_IN_RELATIVE).exists():
        # building from repo root
        config_in = PROJECT_ROOT / CONFIG_IN_RELATIVE
    elif (PROJECT_ROOT / "build" / CONFIG_IN_RELATIVE).exists():
        # building from "build/" directory
        config_in = PROJECT_ROOT / "build" / CONFIG_IN_RELATIVE
    else:
        # unhandled case
        raise FileNotFoundError(f"Could not find anj_config.h")
...
def _main():
    ...
    staging_include_public = doxygen_input_dir / 'include_public/anj'
