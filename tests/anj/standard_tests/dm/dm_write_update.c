...
static bool inst_transaction_end_return_error;
...
static int transaction_validate(anj_t *anj, const anj_dm_obj_t *obj) {
    ...
    if (inst_transaction_end_return_error) {
        return -1;
    }
    return 0;
}
...
ANJ_UNIT_TEST(dm_write_update, write_handler) {
    ...
    ANJ_UNIT_ASSERT_SUCCESS(_anj_dm_operation_begin(
            &anj, _ANJ_OP_DM_WRITE_PARTIAL_UPDATE, false, &path));
...
ANJ_UNIT_TEST(dm_write_update, error_path_1) {
    TEST_INIT(anj, obj);
...
ANJ_UNIT_TEST(dm_write_update, error_path_2) {
    TEST_INIT(anj, obj);
    res_write_operation_error = ANJ_DM_ERR_NOT_FOUND;
    anj_io_out_entry_t record = {
        .type = ANJ_DATA_TYPE_INT,
...
ANJ_UNIT_TEST(dm_write_update, error_path_3) {
    TEST_INIT(anj, obj);
    anj_io_out_entry_t record = {
        .type = ANJ_DATA_TYPE_INT,
...
ANJ_UNIT_TEST(dm_write_update, error_path_4) {
    TEST_INIT(anj, obj);
    anj_io_out_entry_t record = {
        .type = ANJ_DATA_TYPE_INT,
