...
#define SET_UP()                                                     \
    ...
    msg.coap_binding_data.message_id = 0x1111;                       \
    anj_t anj = { 0 };                                               \
...
#define MSG_ID_OFFSET 2
    expected[MSG_ID_OFFSET] = (uint8_t) (msg.coap_binding_data.message_id >> 8);
...
ANJ_UNIT_TEST(dm_integration, discover_operation_root) {
    ...
    char expected[] =
            "\x61"              // ACK, tkl 1
            "\x85\x11\x11\x01"; // 4.05 Method Not Allowed, msg_id token
...
ANJ_UNIT_TEST(dm_integration, read_operation_block_resource_size_changes) {
    ...
    for (int i = 0; i < (int) sizeof(res_4_buff) - 1; i++) {
        res_4_buff[i] = 'a';
...
// Test writing TLV payload to a resource instance using different URI paths
// 111/2/2/1 : 0x37
#define WRITE_UPDATE_WITH_TLV_ON_RESOURCE_INSTANCE_TEST(Uri)       \
    ...
    char expected[] = "\x61"                                       \
                      "\x44\x11\x11\x01";                          \
    verify_payload(expected, sizeof(expected) - 1, &msg);          \
    ANJ_UNIT_ASSERT_EQUAL(write_value, 0x37);                      \
    ANJ_UNIT_ASSERT_EQUAL(                                         \
            transaction_end_results[TRANSACTION_END_RESULT_OBJ_1], \
            ANJ_DM_TRANSACTION_SUCCESS);

ANJ_UNIT_TEST(dm_integration,
              write_update_operation_on_resource_instance_tlv_instance_path) {
    WRITE_UPDATE_WITH_TLV_ON_RESOURCE_INSTANCE_TEST(
            ANJ_MAKE_INSTANCE_PATH(111, 2));
}

ANJ_UNIT_TEST(dm_integration,
              write_update_operation_on_resource_instance_tlv_resource_path) {
    WRITE_UPDATE_WITH_TLV_ON_RESOURCE_INSTANCE_TEST(
            ANJ_MAKE_RESOURCE_PATH(111, 2, 2));
}

ANJ_UNIT_TEST(
        dm_integration,
        write_update_operation_on_resource_instance_tlv_resource_instance_path) {
    WRITE_UPDATE_WITH_TLV_ON_RESOURCE_INSTANCE_TEST(
...
#    else  // ANJ_WITH_LWM2M_CBOR
    msg.content_format = _ANJ_COAP_FORMAT_SENML_CBOR;
    msg.payload = (uint8_t *) "\x81\xa2\x21\x6A"
                              "/111/2/2/3"
                              "\x02\x18\x05";
    msg.payload_size = 17;
#    endif // ANJ_WITH_LWM2M_CBOR
