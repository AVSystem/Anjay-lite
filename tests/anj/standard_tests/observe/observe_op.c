...
#    endif // ANJ_WITH_LWM2M12
        ...
        ASSERT_TRUE(anj_time_monotonic_eq(
                ctx1->observations[i].last_notify_timestamp,
...
#    define OBSERVE_OP(Path, Attr, Result, Msg_Code)                          \
        ...
        inout_msg.coap_binding_data.message_id = 0x1111;                      \
        ...
        ASSERT_EQ(inout_msg.block.block_type, _ANJ_OPTION_BLOCK_NOT_DEFINED); \
...
// This test is similar to the `observe_with_step_when_path_to_res_inst`, but
// checks whether the attribute storage is properly used to obtain attributes
// when observing a resource instance. This case is little different then
// `observe_effective_attr` test, because here we check if value-based
// attributes are properly inherited from resource level to resource instance
// level.
ANJ_UNIT_TEST(observe_op,
              observe_with_step_when_path_to_res_inst_attr_storage) {
    TEST_INIT();
    add_attr_storage(&anj.observe_ctx.attributes_storage[0],
                     ANJ_MAKE_RESOURCE_PATH(3, 1, 8),
                     (_anj_attr_notification_t) {
                         .has_step = true,
                         .step = 2
                     },
    ...
    add_attr_storage(&anj.observe_ctx.attributes_storage[1],
                     ...
                     1);
    OBSERVE_OP_TEST(ANJ_MAKE_RESOURCE_INSTANCE_PATH(3, 1, 8, 1));
...
ANJ_UNIT_TEST(observe_op, observe_check_timestamps) {
    ...
    ctx_ref.observations[0].last_notify_timestamp = anj_time_monotonic_now();
...
#        endif // ANJ_WITH_OBSERVE_COMPOSITE
    ...
    inout_msg.operation = _ANJ_OP_INF_OBSERVE;
...
ANJ_UNIT_TEST(observe_op, observe_block) {
    ...
    uint8_t EXPECTED[] =
            ...
            "\xFF\x11\x11\x22"
            "\x60"                             /*observe = 0*/
            EXPECTED_CONTENT_FORMAT "\xb1\x08" // block2 0
            "\xFF"
    ...
    inout_msg.payload_size = 0;
    inout_msg.coap_binding_data.message_id++;
...
#    endif
    ...
    // for SeNML CBOR we have 1 more chunk to send
#    ifdef ANJ_WITH_LWM2M_CBOR
    ASSERT_EQ(_anj_exchange_process(&exchange_ctx,
                                    ANJ_EXCHANGE_EVENT_SEND_CONFIRMATION,
                                    &inout_msg),
...
ANJ_UNIT_TEST(observe_op, observe_err_no_space) {
    ...
    OBSERVE_OP_TEST_WITH_ERROR(ANJ_MAKE_RESOURCE_INSTANCE_PATH(3, 1, 8, 1),
                               { 0 }, -1, ANJ_COAP_CODE_INTERNAL_SERVER_ERROR);
...
ANJ_UNIT_TEST(observe_op, observe_update_record_error) {
    ...
    OBSERVE_OP_TEST_WITH_ERROR(ANJ_MAKE_RESOURCE_PATH(3, 1, 1), { 0 }, -1,
                               ANJ_COAP_CODE_METHOD_NOT_ALLOWED);
...
#    define CANCEL_OBSERVE_OP_TEST(Path, Result, Expected)                    \
        ...
        _anj_coap_msg_t inout_msg = {                                         \
            .operation = _ANJ_OP_INF_CANCEL_OBSERVE,                          \
            .uri = Path,                                                      \
            .payload_size = 0,                                                \
            .accept = _ANJ_COAP_FORMAT_NOT_DEFINED,                           \
        };                                                                    \
...
ANJ_UNIT_TEST(observe_op, observe_cancel_by_mid_success) {
    TEST_INIT();
    anj.observe_ctx.observations[0].ssid = 1;
    anj.observe_ctx.observations[0].last_sent_mid = 0x12;
    anj.observe_ctx.observations[0].path =
            ANJ_MAKE_RESOURCE_INSTANCE_PATH(3, 1, 8, 1);

    _anj_observe_cancel_observation_by_mid(&anj, 0x12);

    ANJ_UNIT_ASSERT_EQUAL(anj.observe_ctx.observations[0].ssid, 0);
}

ANJ_UNIT_TEST(observe_op, observe_cancel_by_mid_fail) {
    TEST_INIT();
    anj.observe_ctx.observations[0].ssid = 1;
    anj.observe_ctx.observations[0].last_sent_mid = 0x12;
    anj.observe_ctx.observations[0].path =
    ...
    _anj_observe_cancel_observation_by_mid(&anj, 0x34);

    ANJ_UNIT_ASSERT_EQUAL(anj.observe_ctx.observations[0].ssid, 1);
}

ANJ_UNIT_TEST(observe_op, update_mid_found) {
    TEST_INIT();
    anj.observe_ctx.observations[0].ssid = 1;
    anj.observe_ctx.observations[0].last_sent_mid = 0x12;
    anj.observe_ctx.processing_observation = &anj.observe_ctx.observations[0];

    _anj_observe_update_last_mid(&anj, 0x34);

    ANJ_UNIT_ASSERT_EQUAL(anj.observe_ctx.observations[0].last_sent_mid, 0x34);
}

ANJ_UNIT_TEST(observe_op, update_mid_not_found) {
    TEST_INIT();
    anj.observe_ctx.observations[0].ssid = 1;
    anj.observe_ctx.observations[0].last_sent_mid = 0x12;
    anj.observe_ctx.processing_observation = NULL;

    _anj_observe_update_last_mid(&anj, 0x34);

    ANJ_UNIT_ASSERT_EQUAL(anj.observe_ctx.observations[0].last_sent_mid, 0x12);
}
