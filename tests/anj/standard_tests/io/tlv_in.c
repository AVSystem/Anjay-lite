...
ANJ_UNIT_TEST(tlv_in_general_tests, instance_resource_with_base_instance_path) {
    // /111/2/2/1 : 0x37
    char DATA[] = "\x83\x02\x41\x01\x37";
    TEST_ENV(DATA, ANJ_MAKE_INSTANCE_PATH(111, 2), true);
    anj_data_type_t type_bitmask = ANJ_DATA_TYPE_UINT;
    ASSERT_OK(_anj_io_in_ctx_get_entry(&ctx, &type_bitmask, &value, &path));
    anj_uri_path_t expected_path =
            ANJ_MAKE_RESOURCE_INSTANCE_PATH(111, 2, 2, 1);
    ASSERT_TRUE(anj_uri_path_equal(path, &expected_path));
    ASSERT_EQ(_anj_io_in_ctx_get_entry(&ctx, &type_bitmask, &value, &path),
              _ANJ_IO_EOF);
}

ANJ_UNIT_TEST(tlv_in_general_tests, instance_resource_with_base_resource_path) {
    // /111/2/2/1 : 0x37
    char DATA[] = "\x83\x02\x41\x01\x37";
    TEST_ENV(DATA, ANJ_MAKE_RESOURCE_PATH(111, 2, 2), true);
    anj_data_type_t type_bitmask = ANJ_DATA_TYPE_UINT;
    ASSERT_OK(_anj_io_in_ctx_get_entry(&ctx, &type_bitmask, &value, &path));
    anj_uri_path_t expected_path =
            ANJ_MAKE_RESOURCE_INSTANCE_PATH(111, 2, 2, 1);
...
ANJ_UNIT_TEST(tlv_in_general_tests,
              instance_resource_with_base_resource_instance_path) {
    // /111/2/2/1 : 0x37
    char DATA[] = "\x83\x02\x41\x01\x37";
    TEST_ENV(DATA, ANJ_MAKE_RESOURCE_INSTANCE_PATH(111, 2, 2, 1), true);
    anj_data_type_t type_bitmask = ANJ_DATA_TYPE_UINT;
    ASSERT_OK(_anj_io_in_ctx_get_entry(&ctx, &type_bitmask, &value, &path));
    anj_uri_path_t expected_path =
            ANJ_MAKE_RESOURCE_INSTANCE_PATH(111, 2, 2, 1);
...
ANJ_UNIT_TEST(tlv_in_general_tests, no_resource_path_in_tlv_path) {
    // base path: /111/2
    // path in TLV: /1 - so resource id is missing, decoding should fail
    char DATA[] = "\x41\x01\x37";
    TEST_ENV(DATA, ANJ_MAKE_INSTANCE_PATH(111, 2), true);
    anj_data_type_t type_bitmask = ANJ_DATA_TYPE_UINT;
    ASSERT_EQ(_anj_io_in_ctx_get_entry(&ctx, &type_bitmask, &value, &path),
              _ANJ_IO_ERR_FORMAT);
