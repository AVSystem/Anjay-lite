...
ANJ_UNIT_TEST(senml_cbor_encoder,
              single_send_record_with_all_fields_root_path_in_base_path) {
    senml_cbor_test_env_t env = { 0 };
    ...
    ANJ_UNIT_ASSERT_SUCCESS(_anj_io_out_ctx_get_payload(
            &env.ctx, env.buf, env.buffer_length, &env.out_length));
    // base name is empty because base path is root
...
ANJ_UNIT_TEST(senml_cbor_encoder,
              single_send_record_with_all_fields_full_path_in_base_path) {
    senml_cbor_test_env_t env = { 0 };
    ...
    ANJ_UNIT_ASSERT_SUCCESS(_anj_io_out_ctx_get_payload(
            &env.ctx, env.buf, env.buffer_length, &env.out_length));
    // base name is full path so there is no name field
    VERIFY_BYTES(env, "\x81\xA3"
                      "\x21\x66\x2F\x33\x2F\x33\x2F\x33" // base name
                      "\x22\xFA\x47\xC3\x50\x00"         // base time
                      "\x02\x18\x19");
}

ANJ_UNIT_TEST(senml_cbor_encoder,
              single_send_record_with_all_fields_obj_path_in_base_path) {
    senml_cbor_test_env_t env = { 0 };
    ...
    ANJ_UNIT_ASSERT_SUCCESS(_anj_io_out_ctx_get_payload(
            &env.ctx, env.buf, env.buffer_length, &env.out_length));
    // path is split into base name and name
    VERIFY_BYTES(env, "\x81\xA4"
                      "\x21\x62\x2F\x33"         // base name
                      "\x00\x64\x2F\x32\x2F\x31" // name
                      "\x22\xFA\x47\xC3\x50\x00" // base time
                      "\x02\x18\x19");
}

ANJ_UNIT_TEST(senml_cbor_encoder, send_with_two_records_and_base_path) {
    senml_cbor_test_env_t env = { 0 };
    ...
    anj_io_out_entry_t entry_2 = {
        .timestamp = 100000.0,
        .path = ANJ_MAKE_RESOURCE_PATH(3, 2, 2),
        .type = ANJ_DATA_TYPE_UINT,
        .value.uint_value = 26
    };
    ...
    env.out_length += len;
    VERIFY_BYTES(env, "\x82\xA4"
                      "\x21\x64\x2F\x33\x2F\x32" // base name
                      "\x00\x62\x2F\x31"         // name
                      "\x22\xFA\x47\xC3\x50\x00" // base time
                      "\x02\x18\x19"             // value 25
                      "\xA2"
                      "\x00\x62\x2F\x32" // name
                      "\x02\x18\x1A");   // value 26
}
...
ANJ_UNIT_TEST(senml_cbor_encoder, path_outside_of_base_path) {
    ...
    ANJ_UNIT_ASSERT_EQUAL(_anj_io_out_ctx_new_entry(&env.ctx, &entry),
