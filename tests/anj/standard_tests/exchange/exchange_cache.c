...
ANJ_UNIT_TEST(exchange_cache, add_first_response) {
    ...
    anj_time_monotonic_t monotonic_time = anj_time_monotonic_now();

    _anj_coap_msg_t dummy_response = {
        .coap_binding_data.message_id = 0x1234,
        .payload_size = 4,
...
ANJ_UNIT_TEST(exchange_cache, add_response_moves_recent_to_nonrecent) {
    ...
    cache.cache_recent.response.coap_binding_data.message_id = 0xABCD;
    cache.cache_recent.expiration_time =
            anj_time_monotonic_add(monotonic_time,
    ...
    cache.cache_non_recent[0].expiration_time = ANJ_TIME_MONOTONIC_INVALID;

    // new response to add
    _anj_coap_msg_t dummy_response = {
        .coap_binding_data.message_id = 0x4321,
        .payload_size = 4,
...
ANJ_UNIT_TEST(exchange_cache, add_response_uses_expired_nonrecent_slot) {
    ...
    cache.cache_recent.response.coap_binding_data.message_id = 0xAAAA;
    cache.cache_recent.expiration_time =
            anj_time_monotonic_add(monotonic_time,
    ...
    for (size_t i = 0; i < ANJ_ARRAY_SIZE(cache.cache_non_recent); ++i) {
        ...
        cache.cache_non_recent[i].expiration_time = anj_time_monotonic_add(
                monotonic_time,
                anj_time_duration_new(150 - i, ANJ_TIME_UNIT_S));
    }
    ...
    _anj_coap_msg_t dummy_response = {
        .coap_binding_data.message_id = 0xBBBB,
        .payload_size = 4,
    ...
    ASSERT_TRUE(anj_time_monotonic_eq(cache.cache_recent.expiration_time,
                                      expected_expiration));
    ASSERT_EQ(cache.cache_recent.response.payload_size, 4);
...
ANJ_UNIT_TEST(exchange_cache, add_response_overwrites_oldest_valid_nonrecent) {
    ...
    cache.cache_non_recent[1].expiration_time = anj_time_monotonic_add(
            monotonic_time,
    ...
    for (size_t i = 0; i < ANJ_ARRAY_SIZE(cache.cache_non_recent); ++i) {
        ...
        ASSERT_TRUE(anj_time_monotonic_eq(
                ...
                anj_time_monotonic_add(
                        monotonic_time,
                        anj_time_duration_new(125 + i, ANJ_TIME_UNIT_S))));
    }
}
...
ANJ_UNIT_TEST(exchange_cache, adds_new_response_when_recent_is_expired) {
    ...
    cache.cache_recent.response.coap_binding_data.message_id = 0xABCD;
    cache.cache_recent.expiration_time =
            anj_time_monotonic_new(100, ANJ_TIME_UNIT_S);
    ...
    for (size_t i = 0; i < ANJ_ARRAY_SIZE(cache.cache_non_recent); ++i) {
        ...
        cache.cache_non_recent[i].expiration_time = anj_time_monotonic_add(
                monotonic_time,
                anj_time_duration_new(200 + i, ANJ_TIME_UNIT_S));
    }
...
ANJ_UNIT_TEST(exchange_cache, returns_recent_if_mid_matches_recent) {
    ...
    cache.cache_recent.response.coap_binding_data.message_id = 0xABCD;
    cache.cache_recent.expiration_time = anj_time_monotonic_add(
            monotonic_time,
...
ANJ_UNIT_TEST(exchange_cache, returns_no_entry_if_mid_matches_nothing) {
    ...
    cache.cache_recent.response.coap_binding_data.message_id = 0xAAAA;
    cache.cache_recent.expiration_time =
            anj_time_monotonic_new(130, ANJ_TIME_UNIT_S);
...
ANJ_UNIT_TEST(exchange_cache, expired_nonrecent_entry_is_ignored) {
    ...
    cache.cache_non_recent[2].expiration_time =
            anj_time_monotonic_new(120, ANJ_TIME_UNIT_S);
...
ANJ_UNIT_TEST(exchange_cache, copies_recent_response_and_clears_flag) {
    ...
    _anj_coap_msg_t dummy = {
        .coap_binding_data.message_id = 0xDEAD,
        .payload_size = 3,
        .payload = (uint8_t *) "OK\n"
    };
    ...
    cache.cache_recent.expiration_time =
            anj_time_monotonic_new(150, ANJ_TIME_UNIT_S);
    ...
    ASSERT_EQ(output.coap_binding_data.message_id, 0xDEAD);
