...
/**
 * Error code returned by @ref anj_coap_downloader_get_error() when an internal
 * error occurred within the CoAP downloader module.
 *
...
typedef enum {
    ...
    /**
     * A new connection to the server is being established.
     * The download will begin once the connection is ready.
     */
    ANJ_COAP_DOWNLOADER_STATUS_STARTING,
    /**
     * A new chunk of data is being downloaded.
     *
     * This status will be reported via callback for each received packet.
     */
    ...
    /**
     * The download process has finished successfully.
     * The last packet has been received and the connection has been closed.
     */
     ...
     * Call @ref anj_coap_downloader_get_error to retrieve error details.
     */
 ...
 * For all other statuses, @p data will be NULL and @p data_len will be 0.
 *
...
/**
 * Coap downloader configuration structure. Provided in @ref
 * anj_coap_downloader_init() function.
 */
typedef struct anjay_coap_downloader_configuration_struct {
    /**
...
/**
 * Initializes the CoAP downloader module.
 * The @p coap_downloader object must be created and allocated by the user
 * before calling this function.
 *
 * @param coap_downloader  CoAP downloader object to operate on.
 ...
 *
 * @note The URI is not copied or stored internally by the CoAP downloader,
 *       so the pointer must remain valid throughout the entire download
 *       process.
 *
 * @param coap_downloader  CoAP downloader instance to operate on.
 ...
 *
 * @param coap_downloader  CoAP downloader instance to query.
 *
 * @return Error code indicating the reason for the failure.
 */
